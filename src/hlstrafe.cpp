...
namespace HLStrafe
{
	...
	static void SideStrafeGeneral(const PlayerData& player, const MovementVars& vars, PositionType postype, double wishspeed,
		HLTAS::Button& usedButton, double vel_yaw, double theta, bool right, bool safeguard_yaw, float velocities[2][2], double yaws[2], const CurrentState& curState, ProcessedFrame& out)
	{
		assert(postype != PositionType::WATER);
		...
		if (curState.Algorithm == HLTAS::StrafingAlgorithm::YAW) {
			if (curState.ButtonsPresent) {
				...
				if (curState.Version >= 2 && IsZero<float, 2>(player.Velocity))
	...
	double SideStrafeMaxAccel(PlayerData& player, const MovementVars& vars, PositionType postype, double wishspeed, HLTAS::Button& usedButton,
		double vel_yaw, bool right, const CurrentState& curState, ProcessedFrame& out)
	{
		assert(postype != PositionType::WATER);
		...
		double yaws[2];
		SideStrafeGeneral(player, vars, postype, wishspeed, usedButton, vel_yaw, theta, right, false, velocities, yaws, curState, out);
		...
		double vel_yaw, bool right, const CurrentState& curState, ProcessedFrame& out)
	{
		...
		float velocities[2][2];
		double yaws[2];
		SideStrafeGeneral(player, vars, postype, wishspeed, usedButton, vel_yaw, theta, right, safeguard_yaw, velocities, yaws, curState, out);
	...
	double SideStrafeMaxDeccel(PlayerData& player, const MovementVars& vars, PositionType postype, double wishspeed, HLTAS::Button& usedButton,
		double vel_yaw, bool right, bool& strafed, const CurrentState& curState, ProcessedFrame& out)
	{
		assert(postype != PositionType::WATER);
	...
	double BestStrafeMaxAccel(PlayerData& player, const MovementVars& vars, PositionType postype, double wishspeed, HLTAS::Button& usedButton,
		double vel_yaw, const CurrentState& curState, ProcessedFrame& out)
	{
		assert(postype != PositionType::WATER);
		...
		yaws[0] = SideStrafeMaxAccel(player, vars, postype, wishspeed, buttons[0], vel_yaw, false, curState, out);
		VecCopy<float, 2>(player.Velocity, temp_vel);
		VecCopy<float, 2>(orig_vel, player.Velocity);
		yaws[1] = SideStrafeMaxAccel(player, vars, postype, wishspeed, buttons[1], vel_yaw, true, curState, out);
		...
		double vel_yaw, const CurrentState& curState, ProcessedFrame& out)
	{
		...
		HLTAS::Button buttons[2];
		VecCopy<float, 2>(player.Velocity, orig_vel);
		yaws[0] = SideStrafeMaxAngle(player, vars, postype, wishspeed, buttons[0], vel_yaw, false, curState, out);
		VecCopy<float, 2>(player.Velocity, temp_vel);
		VecCopy<float, 2>(orig_vel, player.Velocity);
		yaws[1] = SideStrafeMaxAngle(player, vars, postype, wishspeed, buttons[1], vel_yaw, true, curState, out);
	...
	double BestStrafeMaxDeccel(PlayerData& player, const MovementVars& vars, PositionType postype, double wishspeed, HLTAS::Button& usedButton,
		double vel_yaw, bool& strafed, const CurrentState& curState, ProcessedFrame& out)
	{
		assert(postype != PositionType::WATER);
		...
		HLTAS::Button buttons[2];
		VecCopy<float, 2>(player.Velocity, orig_vel);
		yaws[0] = SideStrafeMaxDeccel(player, vars, postype, wishspeed, buttons[0], vel_yaw, false, strafed, curState, out);
		VecCopy<float, 2>(player.Velocity, temp_vel);
		VecCopy<float, 2>(orig_vel, player.Velocity);
		yaws[1] = SideStrafeMaxDeccel(player, vars, postype, wishspeed, buttons[1], vel_yaw, true, strafed, curState, out);
		...
		double vel_yaw, const CurrentState& curState, ProcessedFrame& out)
	{
		...
		HLTAS::Button buttons[2];
		VecCopy<float, 2>(player.Velocity, orig_vel);
		yaws[0] = SideStrafeConstSpeed(player, vars, postype, wishspeed, buttons[0], vel_yaw, false, curState, out);
		VecCopy<float, 2>(player.Velocity, temp_vel);
		VecCopy<float, 2>(orig_vel, player.Velocity);
		yaws[1] = SideStrafeConstSpeed(player, vars, postype, wishspeed, buttons[1], vel_yaw, true, curState, out);
	...
	double YawStrafeMaxAccel(PlayerData& player, const MovementVars& vars, PositionType postype, double wishspeed, HLTAS::Button& usedButton,
		double vel_yaw, double yaw, const CurrentState& curState, ProcessedFrame& out)
	{
		assert(postype != PositionType::WATER);
		...
		float velocities[2][2];
		double yaws[2];
		SideStrafeGeneral(player, vars, postype, wishspeed, usedButton, vel_yaw, std::fabs(theta), (theta < 0), false, velocities, yaws, curState, out);
		...
		double vel_yaw, double yaw, const CurrentState& curState, ProcessedFrame& out)
	{
		...
		if (!IsZero<float, 2>(player.Velocity))
			vel_yaw = Atan2(player.Velocity[1], player.Velocity[0]);
		SideStrafeGeneral(player, vars, postype, wishspeed, usedButton, vel_yaw, theta, (NormalizeRad(yaw - vel_yaw) < 0), safeguard_yaw, velocities, yaws, curState, out);
	...
	double YawStrafeMaxDeccel(PlayerData& player, const MovementVars& vars, PositionType postype, double wishspeed, HLTAS::Button& usedButton,
		double vel_yaw, double yaw, bool& strafed, const CurrentState& curState, ProcessedFrame& out)
	{
		assert(postype != PositionType::WATER);
		...
		double vel_yaw, double yaw, const CurrentState& curState, ProcessedFrame& out)
	{
		...
		float velocities[2][2];
		double yaws[2];
		SideStrafeGeneral(player, vars, postype, wishspeed, usedButton, vel_yaw, theta, (NormalizeRad(yaw - vel_yaw) < 0), false, velocities, yaws, curState, out);
	...
	double PointStrafe(PlayerData& player, const MovementVars& vars, PositionType postype, double wishspeed, HLTAS::Button& usedButton,
		double vel_yaw, HLTAS::StrafeType type, double point[2], bool& strafed, const CurrentState& curState, ProcessedFrame& out)
	{
		assert(postype != PositionType::WATER);
		...
		switch (type) {
		default:
		case HLTAS::StrafeType::MAXACCEL: return YawStrafeMaxAccel(player, vars, postype, wishspeed, usedButton, vel_yaw, yaw, curState, out);
		case HLTAS::StrafeType::MAXANGLE: return YawStrafeMaxAngle(player, vars, postype, wishspeed, usedButton, vel_yaw, yaw, curState, out);
		case HLTAS::StrafeType::MAXDECCEL: return YawStrafeMaxDeccel(player, vars, postype, wishspeed, usedButton, vel_yaw, yaw, strafed, curState, out);
		case HLTAS::StrafeType::CONSTSPEED: return YawStrafeConstSpeed(player, vars, postype, wishspeed, usedButton, vel_yaw, yaw, curState, out);
	...
	void Jumpbug(const PlayerData& player, const MovementVars& vars, PositionType postype, const HLTAS::Frame& frame, ProcessedFrame& out, CurrentState& curState, TraceFunc traceFunc)
	{
		assert(postype != PositionType::WATER);
		...
		postype = Strafe(playerCopy, vars, postype, frame, outCopy, false, true, curState, traceFunc);
		if (postype == PositionType::GROUND) {
	...
	void Dbc(const PlayerData& player, const MovementVars& vars, PositionType postype, const HLTAS::Frame& frame, ProcessedFrame& out, CurrentState& curState, TraceFunc traceFunc)
	{
		assert(postype != PositionType::WATER);
		...
		Strafe(playerCopy, vars, postype, frame, out_temp, false, true, curState, traceFunc, fractionsUnducked, normalzsUnducked);
		...
		Strafe(playerCopy, vars, postype, frame, out_temp, false, true, curState, traceFunc, fractionsDucked);
	...
	void Dbg(const PlayerData& player, const MovementVars& vars, PositionType postype, const HLTAS::Frame& frame, ProcessedFrame& out, CurrentState& curState, TraceFunc traceFunc)
	{
		assert(postype != PositionType::WATER);
		...
		postype = Strafe(playerCopy, vars, postype, frame, out_temp, false, true, curState, traceFunc, nullptr, normalzsUnducked);
	...
	void LgagstDucktap(const PlayerData& player, const MovementVars& vars, PositionType postype, const HLTAS::Frame& frame, ProcessedFrame& out, bool reduceWishspeed, CurrentState& curState, TraceFunc traceFunc)
	{
		assert(postype != PositionType::WATER);
		...
		auto out_temp = ProcessedFrame(out);
		Strafe(playerCopy, vars, postype, frame, out_temp, reduceWishspeed, true, curState, traceFunc);
		...
		out_temp = ProcessedFrame(out);
		Strafe(ground, vars, postype, frame, out_temp, false, false, curState, traceFunc);
		...
		Strafe(air, vars, postype, frame, out_temp, false, false, curState, traceFunc);
	...
	void LgagstJump(const PlayerData& player, const MovementVars& vars, PositionType postype, const HLTAS::Frame& frame, ProcessedFrame& out, bool reduceWishspeed, CurrentState& curState, TraceFunc traceFunc)
	{
		assert(postype != PositionType::WATER);
		...
		CheckVelocity(ground, vars);
		auto out_temp = ProcessedFrame(out);
		Strafe(ground, vars, postype, frame, out_temp, reduceWishspeed && !curState.LgagstFullMaxspeed, false, curState, traceFunc);
		...
		Strafe(air, vars, postype, frame, out_temp, reduceWishspeed && !curState_temp.LgagstFullMaxspeed, false, curState, traceFunc);
	...
	PositionType Strafe(PlayerData& player, const MovementVars& vars, PositionType postype, const HLTAS::Frame& frame, ProcessedFrame& out, bool reduceWishspeed, bool predictOrigin, const CurrentState& curState, TraceFunc traceFunc, float fractions[4], float normalzs[4])
	{
		...
		if (frame.Strafe) {
			...
			auto vel_yaw = (curState.Version >= 2) ? 0 : out.Yaw * M_DEG2RAD;

			switch (frame.GetDir()) {
			case HLTAS::StrafeDir::LEFT:
				if (frame.GetType() == HLTAS::StrafeType::MAXACCEL)
					out.Yaw = static_cast<float>(SideStrafeMaxAccel(player, vars, postype, wishspeed, usedButton, vel_yaw, false, curState, out) * M_RAD2DEG);
				else if (frame.GetType() == HLTAS::StrafeType::MAXANGLE)
					out.Yaw = static_cast<float>(SideStrafeMaxAngle(player, vars, postype, wishspeed, usedButton, vel_yaw, false, curState, out) * M_RAD2DEG);
				else if (frame.GetType() == HLTAS::StrafeType::CONSTSPEED)
					out.Yaw = static_cast<float>(SideStrafeConstSpeed(player, vars, postype, wishspeed, usedButton, vel_yaw, false, curState, out) * M_RAD2DEG);
				else if (frame.GetType() == HLTAS::StrafeType::MAXDECCEL) {
					auto yaw = static_cast<float>(SideStrafeMaxDeccel(player, vars, postype, wishspeed, usedButton, vel_yaw, false, strafed, curState, out) * M_RAD2DEG);
					if (strafed)
						out.Yaw = yaw;
				}
				break;
			...
			case HLTAS::StrafeDir::RIGHT:
				if (frame.GetType() == HLTAS::StrafeType::MAXACCEL)
					out.Yaw = static_cast<float>(SideStrafeMaxAccel(player, vars, postype, wishspeed, usedButton, vel_yaw, true, curState, out) * M_RAD2DEG);
				else if (frame.GetType() == HLTAS::StrafeType::MAXANGLE)
					out.Yaw = static_cast<float>(SideStrafeMaxAngle(player, vars, postype, wishspeed, usedButton, vel_yaw, true, curState, out) * M_RAD2DEG);
				else if (frame.GetType() == HLTAS::StrafeType::CONSTSPEED)
					out.Yaw = static_cast<float>(SideStrafeConstSpeed(player, vars, postype, wishspeed, usedButton, vel_yaw, true, curState, out) * M_RAD2DEG);
				else if (frame.GetType() == HLTAS::StrafeType::MAXDECCEL) {
					auto yaw = static_cast<float>(SideStrafeMaxDeccel(player, vars, postype, wishspeed, usedButton, vel_yaw, true, strafed, curState, out) * M_RAD2DEG);
					if (strafed)
						out.Yaw = yaw;
				}
				break;
			...
			case HLTAS::StrafeDir::BEST:
				if (frame.GetType() == HLTAS::StrafeType::MAXACCEL)
					out.Yaw = static_cast<float>(BestStrafeMaxAccel(player, vars, postype, wishspeed, usedButton, vel_yaw, curState, out) * M_RAD2DEG);
				else if (frame.GetType() == HLTAS::StrafeType::MAXANGLE)
					out.Yaw = static_cast<float>(BestStrafeMaxAngle(player, vars, postype, wishspeed, usedButton, vel_yaw, curState, out) * M_RAD2DEG);
				else if (frame.GetType() == HLTAS::StrafeType::CONSTSPEED)
					out.Yaw = static_cast<float>(BestStrafeConstSpeed(player, vars, postype, wishspeed, usedButton, vel_yaw, curState, out) * M_RAD2DEG);
				else if (frame.GetType() == HLTAS::StrafeType::MAXDECCEL) {
					auto yaw = static_cast<float>(BestStrafeMaxDeccel(player, vars, postype, wishspeed, usedButton, vel_yaw, strafed, curState, out) * M_RAD2DEG);
					if (strafed)
						out.Yaw = yaw;
				}
				break;
			...
			case HLTAS::StrafeDir::YAW:
				if (frame.GetType() == HLTAS::StrafeType::MAXACCEL)
					out.Yaw = static_cast<float>(YawStrafeMaxAccel(player, vars, postype, wishspeed, usedButton, vel_yaw, frame.GetYaw() * M_DEG2RAD, curState, out) * M_RAD2DEG);
				else if (frame.GetType() == HLTAS::StrafeType::MAXANGLE)
					out.Yaw = static_cast<float>(YawStrafeMaxAngle(player, vars, postype, wishspeed, usedButton, vel_yaw, frame.GetYaw() * M_DEG2RAD, curState, out) * M_RAD2DEG);
				else if (frame.GetType() == HLTAS::StrafeType::CONSTSPEED)
					out.Yaw = static_cast<float>(YawStrafeConstSpeed(player, vars, postype, wishspeed, usedButton, vel_yaw, frame.GetYaw() * M_DEG2RAD, curState, out) * M_RAD2DEG);
				else if (frame.GetType() == HLTAS::StrafeType::MAXDECCEL) {
					auto yaw = static_cast<float>(YawStrafeMaxDeccel(player, vars, postype, wishspeed, usedButton, vel_yaw, frame.GetYaw() * M_DEG2RAD, strafed, curState, out) * M_RAD2DEG);
					if (strafed)
						out.Yaw = yaw;
				}
				break;
			...
			case HLTAS::StrafeDir::POINT:
			{
				...
				auto yaw = static_cast<float>(PointStrafe(player, vars, postype, wishspeed, usedButton, vel_yaw, frame.GetType(), point, strafed, curState, out) * M_RAD2DEG);
				if (strafed)
					out.Yaw = yaw;
			}
				break;
	...
	void CheckIfNextFrameShouldBe0ms(const PlayerData& player, const MovementVars& vars, const HLTAS::Frame& frame, PositionType postype, ProcessedFrame& out, const CurrentState& curState, TraceFunc traceFunc)
	{
		if (postype != PositionType::GROUND
	...
	ProcessedFrame MainFunc(const PlayerData& player, const MovementVars& vars, const HLTAS::Frame& frame, CurrentState& curState, TraceFunc traceFunc)
	{
		...
		Jumpbug(playerCopy, vars, postype, frame, out, curState, traceFunc);
		Dbc(playerCopy, vars, postype, frame, out, curState, traceFunc);
		Dbg(playerCopy, vars, postype, frame, out, curState, traceFunc);
		LgagstDucktap(playerCopy, vars, postype, frame, out, reduceWishspeed, curState, traceFunc);
		...
		LgagstJump(playerCopy, vars, postype, frame, out, reduceWishspeed, curState, traceFunc);
		Autojump(postype, frame, curState, out);
		postype = PredictJump(playerCopy, postype, vars, frame, curState, out, traceFunc, true);
		Friction(playerCopy, postype, vars, traceFunc);
		CheckVelocity(playerCopy, vars);
		postype = Strafe(playerCopy, vars, postype, frame, out, reduceWishspeed, true, curState, traceFunc, out.fractions, out.normalzs);

		CheckIfNextFrameShouldBe0ms(playerCopy, vars, frame, postype, out, curState, traceFunc);
